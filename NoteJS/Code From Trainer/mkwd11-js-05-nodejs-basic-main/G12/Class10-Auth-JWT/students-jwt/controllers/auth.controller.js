...
export class AuthController {
  ...
  static async loginUser(req, res) {
    try {
      ...
      // 1. Creating token is done after success with login
      const accessToken = createAccessToken(user.id);
      
      // 2. We set the access-token in the header with accessToken value
      res.set("access-token", accessToken);

      // 3. Creating a refresh token after succesfull login
      const refreshToken = createRefreshToken(user.id);

      // 4. We save the refresh token in the database
      await AuthModel.saveRefreshToken(user.id, refreshToken);

      // 5.Then we also set refresh-token with value refreshToken after login
